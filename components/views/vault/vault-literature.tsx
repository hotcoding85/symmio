...
export function VaultLiteratureSection({ literature }: VaultLiteratureProps) {
  if (literature.length === 0) {
    return (
      <div className="bg-[#202426] rounded-lg p-6 border border-[#afafaf1a] text-center">
      ...
      <div className="grid custom-3xl-grid gap-x-8 gap-y-6">
        {literature.map((doc) => (
          <Link
            ...
            className="group flex gap-3 items-center hover:opacity-80 transition-opacity"
          >
            ...
            <div className="pt-0">
              <h3 className="text-white text-[12px] font-normal group-hover:text-blue-400 transition-colors leading-[16px] overflow-hidden text-ellipsis whitespace-normal line-clamp-3">
                {doc.name}
              </h3>
            </div>
          </Link>
